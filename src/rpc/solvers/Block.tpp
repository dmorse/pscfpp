...
namespace Rp {
   ...
   */
   template <int D>
   void Block<D, Rpc::Types<D> >::associate(Mesh<D> const & mesh,
                            FFT<D> const& fft,
   ...
   */
   template <int D>
   void Block<D, Rpc::Types<D> >::allocate(double ds)
   {
      UTIL_CHECK(ds > 0.0);
      UTIL_CHECK(meshPtr_);
      UTIL_CHECK(fftPtr_);
   ...
   */
   template <int D>
   void Block<D, Rpc::Types<D> >::setLength(double newLength)
   {
   ...
   */
   template <int D>
   void Block<D, Rpc::Types<D> >::setKuhn(double kuhn)
   {
   ...
   */
   template <int D>
   void Block<D, Rpc::Types<D> >::clearUnitCellData()
   {  
      hasExpKsq_ = false;
      stress_.clear();
   }
   ...
   */
   template <int D>
   void Block<D, Rpc::Types<D> >::computeExpKsq()
   {
      UTIL_CHECK(isAllocated_);
      UTIL_CHECK(waveListPtr_);
   ...
   */
   template <int D>
   void
   Block<D, Rpc::Types<D> >::setupSolver(RField<D> const& w)
   {
      // Preconditions
      int nx = mesh().size();
      UTIL_CHECK(nx > 0);
   ...
   */
   template <int D>
   void Block<D, Rpc::Types<D> >::stepThread(RField<D> const & q, RField<D>& qout) const
   {
      UTIL_CHECK(PolymerModel::isThread());
   ...
   */
   template <int D>
   void Block<D, Rpc::Types<D> >::stepBead(RField<D> const & q, RField<D>& qout) const
   {
   ...
   */
   template <int D>
   void Block<D, Rpc::Types<D> >::stepBondBead(RField<D> const & q, RField<D>& qout) const
   {
      // Prereconditions 
      UTIL_CHECK(isAllocated_);
      UTIL_CHECK(hasExpKsq_);
   ...
   */
   template <int D>
   void Block<D, Rpc::Types<D> >::stepHalfBondBead(RField<D> const & q, RField<D>& qout) const
   {
      // Preconditions 
      UTIL_CHECK(isAllocated_);
      UTIL_CHECK(hasExpKsq_);
   ...
   */
   template <int D>
   void Block<D, Rpc::Types<D> >::stepFieldBead(RField<D>& q) const
   {
      // Preconditions 
      int nx = mesh().size();
      UTIL_CHECK(nx > 0);
   ...
   */
   template <int D>
   void Block<D, Rpc::Types<D> >::computeConcentrationThread(double prefactor)
   {
      // Preconditions
      UTIL_CHECK(isAllocated_);
      int nx = mesh().size();
      ...
      Rp::Propagator<D, Rpc::Types<D> > const & p0 = propagator(0);
      Rp::Propagator<D, Rpc::Types<D> > const & p1 = propagator(1);
   ...
   */
   template <int D>
   void Block<D, Rpc::Types<D> >::computeConcentrationBead(double prefactor)
   {
      // Preconditions
      UTIL_CHECK(isAllocated_);
      int nx = mesh().size();
   ...
   */
   template <int D>
   void Block<D, Rpc::Types<D> >::computeStressThread(double prefactor)
   {
      // Preconditions
      UTIL_CHECK(PolymerModel::isThread());
      int nx = mesh().size();
   ...
   */
   template <int D>
   void Block<D, Rpc::Types<D> >::computeStressBead(double prefactor)
   {
      // Preconditions
      UTIL_CHECK(PolymerModel::isBead());
      int nx = mesh().size();
