...
namespace Util 
{
   ...
   * Default constructor.
   *
   * Uses default seed only if this is the first instance
   */
   MTRand_int32::MTRand_int32() 
    : p(0),
      init(false)
   { 
      for (int i=0; i< n; ++i) state[i] = 0x0UL;
      if (!init) seed(5489UL); 
      init = true; 
   }
   ...
   */
   MTRand_int32::MTRand_int32(const unsigned long* array, int size) 
    : p(0),
      init(false)
   { 
      for (int i=0; i< n; ++i) state[i] = 0x0UL;
   ...
   * Copy constructor.
   */
   MTRand_int32::MTRand_int32(const MTRand_int32& other) 
    : p(other.p),
      init(other.init)
   { 
      for (int i=0; i< n; ++i) {
   ...
   */
   void MTRand_int32::operator = (const MTRand_int32& other) 
   { 
      p = other.p;
      init = other.init;
      for (int i=0; i< n; ++i) {
