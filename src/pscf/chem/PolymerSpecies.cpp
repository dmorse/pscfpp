...
namespace Pscf
{
   ...
   */
   ...
   void PolymerSpecies::readParameters(std::istream& in)
   {
      ...
      // Allocate vertices_ and propagatorIds_ arrays
      ...
      if (type_ == PolymerType::Linear) {
         for (int edgeId = 0; edgeId < nBlock_; ++edgeId) {
            edge(edgeId).setVertexIds(edgeId, edgeId + 1);
         }
      }
      ...
      // Construct a plan for the order in which propagators
   ...
   /*
   * Make a plan for the order of computation of propagators.
   */
