...
__host__ cudaReal gpuSum(cudaReal const * d_in, int size) 
{
   ...
   // temp_h will handle excess and reduced and sum them up
   HostDArray<cudaReal> temp_h(nBlocks);
   DeviceArray<cudaReal> temp(nBlocks);

   // Perform parallel reduction sum operation
   reductionSum<<<nBlocks, nThreads, nThreads*sizeof(cudaReal)>>>
                                       (temp.cArray(), d_in, size);

   // Load the partially reduced sum result to temp_h
   temp_h = temp;

   // Perform final sum using kahan summation to reduce accumulation of error
...
__host__ cudaReal gpuInnerProduct(cudaReal const * d_a, cudaReal const * d_b, 
                                  int size)
{
   ...
   reductionInnerProduct<<<nBlocks, nThreads, nThreads*sizeof(cudaReal)>>>
                                             (temp.cArray(), d_a, d_b, size);

   // Load the partially reduced inner product result to temp_h
   temp_h = temp;
...
__host__ cudaReal gpuMax(cudaReal const * d_in, int size)
{
   ...
   // Load the partially reduced maximum result to temp_h
   temp_h = temp;
...
__host__ cudaReal gpuMaxAbs(cudaReal const * d_in, int size)
{
   ...
   // Load the partially reduced maximum of the absolute value result to temp_h
   temp_h = temp;

   // Perform final comparison on CPU. Absolute values already taken in 
   // kernel, so no need to take them here.
   cudaReal max = 0;
   for (int i = 0; i < nBlocks; i++) {
      if (temp_h[i] > max) max = temp_h[i];
   }
...
__host__ cudaReal gpuMin(cudaReal const * d_in, int size)
{
   ...
   // Load the partially reduced minimum result to temp_h
   temp_h = temp;
...
__host__ cudaReal gpuMinAbs(cudaReal const * d_in, int size)
{
   ...
   reductionMinAbs<<<nBlocks, nThreads, nThreads*sizeof(cudaReal)>>>
                                          (temp.cArray(), d_in, size);

   // Load the partially reduced minimum of the absolute value result to temp_h
   temp_h = temp;
