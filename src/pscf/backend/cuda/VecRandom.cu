...
namespace Pscf {
   ...
   */
   void VecRandom<CUT>::setSeed(unsigned long long seed)
   {
      if (seed == 0) {
         timeval time;
   ...
   */
   void VecRandom<CUT>::errorCheck(curandStatus_t const & error)
   {
      if (error == CURAND_STATUS_SUCCESS) {
         ...
         Log::file() << "VecRandom error: " << errString << std::endl;
         UTIL_THROW("VecRandom number generation failed.");
      }
   }
