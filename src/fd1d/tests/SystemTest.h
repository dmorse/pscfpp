...
public:
   ...
   void testSolve()
   {
      ...
      for (int i = 0; i < nx; ++i) {
         ...
         mixture.wField(0)[i] = 0.5 + cs;
         mixture.wField(1)[i] = 0.7 - cs;
      }
      mixture.compute();

      // Test if same Q is obtained from different methods
      std::cout << mixture.polymer(0).propagator(0, 0).computeQ() << "\n";
      std::cout << mixture.polymer(0).propagator(1, 0).computeQ() << "\n";
      std::cout << mixture.polymer(0).propagator(1, 1).computeQ() << "\n";
      std::cout << mixture.polymer(0).propagator(0, 1).computeQ() << "\n";

      // Test spatial integral of block concentration
      double sum0 = 0.0;
      double sum1 = 0.0;
      for (int i = 0; i < nx; ++i) {
         sum0 += mixture.cField(0)[i];
         sum1 += mixture.cField(1)[i];
      }
      sum0 = sum0/double(nx);
      sum1 = sum1/double(nx);
