#ifndef PSSP_GPU_UNIT_CELL_BASE_H
#define PSSP_GPU_UNIT_CELL_BASE_H
...
namespace Pssp_gpu { 
   ...
   */
   ...
   public:
      ...
      */
      cufftReal dkkBasis(int rank) const;
   ...
   */
   ...
   template <int D>
   inline cufftReal UnitCellBase<D>::dkkBasis(int rank) const
   {  return dkkBasis_[rank];  }
   ...
   */
   ...
   void UnitCellBase<D>::computeDerivatives()
   {
      ...
      //change dkk to a contiguous array
      int rank = 0;
      for (p = 0; p < nParameter_; ++p) {
         for (q = 0; q < D; ++q) {
            for (r = 0; r < D; ++r) {
               for (s = 0; s < D; ++s) {
                  rank = r + (q * (D)) + (p * (D * D));
                  drrBasis_[rank] += rBasis_[q][s]*drBasis_[p](r,s);
                  drrBasis_[rank] += rBasis_[r][s]*drBasis_[p](q,s);
                  dkkBasis_[rank] += kBasis_[q][s]*dkBasis_[p](r,s);
                  dkkBasis_[rank] += kBasis_[r][s]*dkBasis_[p](q,s);
      ...
      cudaMemcpy(dkkBasis_d, dkkBasis_, 
                 sizeof(cufftReal) * nParameter_ * D * D, 
