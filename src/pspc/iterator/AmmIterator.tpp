#ifndef PSPC_AMM_ITERATOR_TPP
#define PSPC_AMM_ITERATOR_TPP
...
namespace Pspc
{
   ...
   */
   template <int D>
   AmmIterator<D>::AmmIterator(System<D>& system)
    : Iterator<D>(system),
      epsilon_(0),
      lambda_(0),
      nHist_(0),
      maxHist_(0)
   {  setClassName("AmmIterator"); }

   /*
   * Destructor
   */
   template <int D>
   AmmIterator<D>::~AmmIterator()
   {}
   ...
   */
   template <int D>
   void AmmIterator<D>::readParameters(std::istream& in)
   {
   ...
   */
   template <int D>
   int AmmIterator<D>::solve()
   {
      // Preconditions:
      UTIL_CHECK(system().hasWFields());
      // Assumes basis.makeBasis() has been called
      // Assumes AmmIterator.allocate() has been called
   ...
   template <int D>
   void AmmIterator<D>::computeResidual()
   {
      // Relevant quantities
   ...
   template <int D>
   bool AmmIterator<D>::isConverged()
   {
      const int nMonomer = system().mixture().nMonomer();
      const int nParameter = system().unitCell().nParameter();
      const int nBasis = system().basis().nBasis();
   ...
   template <int D>
   void AmmIterator<D>::minimizeCoeff(int itr)
   {
      const int nMonomer = system().mixture().nMonomer();
      const int nParameter = system().unitCell().nParameter();
      const int nBasis = system().basis().nBasis();
   ...
   template <int D>
   void AmmIterator<D>::buildOmega(int itr)
   {
      const int nMonomer = system().mixture().nMonomer();
      const int nParameter = system().unitCell().nParameter();
      const int nBasis = system().basis().nBasis();
   ...
   template <int D>
   bool AmmIterator<D>::isCanonical()
   {
