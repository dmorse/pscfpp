...
#include <rpg/field/FilmFieldGenExt.h>
...
public:
   ...
   void testUpdate() // test FieldGenerator::update()
   {
      ...
      DArray<DArray<double> > hFieldsCheck; // Copy of reference field
      system.domain().fieldIo().readFieldsBasis("in/film/hRef2.bf", hFieldsCheck, 
		                                unitCell);
      BFieldComparison bComparison(0); // object to compare fields
   ...
   void testStress() // test FilmFieldGenExt::stress
   {
      ...
      system.mask().readBasis("in/film/maskRef2.bf",tmpUnitCell);
